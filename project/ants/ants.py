...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        #! the most right place is the beehive
        count = 0
        tmp_place = self.place
        while count < self.min_range:
            if tmp_place == beehive:
                return None
            tmp_place = tmp_place.entrance
            count += 1
        while len(tmp_place.bees) == 0:
            if not tmp_place.entrance or tmp_place.entrance == beehive:
                return None
            if count >= self.max_range:
                return None
            tmp_place = tmp_place.entrance
            count += 1
        return random_or_none(tmp_place.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
        ...
        if self.is_queen:
            ...
            while now_place:
                if now_place.ant:#! 有bug
                    ...
                    if isinstance(now_place.ant, ContainerAnt):
                        if now_place.ant.contained_ant and now_place.ant.contained_ant.count == 0:
                            now_place.ant.contained_ant.damage *= 2
                            now_place.ant.contained_ant.count += 1
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        is_any_ant = self.place.ant is not None
        if is_any_ant:
            is_ninjaant = self.place.ant.blocks_path
            return is_ninjaant
        return is_any_ant
        # END Problem 7
